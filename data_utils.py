...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size,
  ...
  token in the first line gets id=0, secodnd line gets id=1, and so on.
...
def words_to_token_ids(words, vocabulary):
  sentence = " ".join(words)
  return sentence_to_token_ids(sentence, vocabulary, UNK_ID_dict['with_padding'])
...
def get_vocab(data_dir, in_vocab_size, out_vocab_size):
    train_path = data_dir + '/train/train'
    dev_path = data_dir + '/valid/valid'
    test_path = data_dir + '/test/test'

    # Get path of vocabularies.
    in_vocab_path = os.path.join(data_dir, "in_vocab_%d.txt" % in_vocab_size)

    create_vocabulary(in_vocab_path, train_path + ".seq", in_vocab_size,
                      tokenizer=naive_tokenizer, normalize_digits=False)

    in_seq_train_ids_path = train_path + (".ids%d.seq" % in_vocab_size)
    out_seq_train_ids_path = train_path + (".ids%d.seq.out" % out_vocab_size)
    data_to_token_ids(train_path + ".seq", in_seq_train_ids_path, in_vocab_path,
                      tokenizer=naive_tokenizer, normalize_digits=False)

    # Create the same token ids with slu-rnn for the development data.
    in_seq_dev_ids_path = dev_path + (".ids%d.seq" % in_vocab_size)
    # data_to_token_ids(dev_path + ".seq.in", in_seq_dev_ids_path, in_vocab_path, tokenizer=naive_tokenizer)

    # Create the same token ids with slu-rnn for the test data
    in_seq_test_ids_path = test_path + (".ids%d.seq" % in_vocab_size)
    ...
    return (in_seq_train_ids_path, out_seq_train_ids_path,
            in_seq_dev_ids_path, in_seq_test_ids_path, in_vocab_path)